...
import { getAvatarBorderColor } from "../utils/userUtils";
...
export default function UserAvatar({
	...
	const borderColor = getAvatarBorderColor(user);

	return (
		<div className="relative flex w-full overflow-visible h-64">
			<div
				className={`border-4 ${borderColor} rounded-full ml-40 relative z-10 w-64 h-64 bg-arcade-white overflow-hidden flex items-center justify-center shrink-0`}
			>
				{user?.profile_picture ? (
					<img
						src={`${apiUrl}/proxy/profile-image?url=${encodeURIComponent(user.profile_picture)}`}
						alt="Profile Picture"
