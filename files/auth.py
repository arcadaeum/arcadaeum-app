...
async def change_my_username(
    ...
    """Update the current user's unique username after password verification."""
    # 1. Verify password
    ...
    # 2. Try update
    success = update_username(current_user.id, req.new_username)
    if not success:
        raise HTTPException(
    ...
    updated = get_user_by_email(current_user.email)
    return User(**updated)
